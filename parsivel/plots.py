...
def set_ticks_and_labels_xaxis(ax: Axes, dx_seconds: float = 30):
    n_ticks = 7
    b0, bf = ax.get_xbound()
    b0, bf = int(ceil(b0 / 3600) * 3600), int(floor(bf / 3600) * 3600)
    dx_ticks = (bf - b0) / (n_ticks - 1)
    ticks = [b0 + i * dx_ticks for i in range(n_ticks)]
    dx_ticks_hours = dx_ticks / 3600
    ticks_labels = [f"{i*dx_ticks_hours:.1f}h" for i in range(n_ticks)]
    ax.set_xticks(ticks)
    ax.set_xticklabels(ticks_labels)
